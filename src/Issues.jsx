...
function Issues() {
  ...
  const fetchIssues = async () => {
    const res = await fetch(`https://api.github.com/repos/facebook/create-react-app/issues?per_page=10&state=${filter}`);
    return await res.json();
  }
  const {isLoading, isError, isSuccess, data: issues} = useQuery(['issues', filter], fetchIssues)
  return (
    <div>
        ...
        <div className="issues-heading">
          ...
          <div className="open-closed-buttons">
            <button onClick={() => setFilter('open')}>
              <IconOpen />
              <span className={filter === 'open' ? 'font-bold': undefined}>Open</span>
            </button>
            <button onClick={() => setFilter('closed')}>
              <IconClosed />
              <span className={filter === 'closed' ? 'font-bold': undefined}>Closed</span>
            </button>
          </div>
        </div>
              ...
              <div className="issues-entry-title-container">
                ...
                <div className="issues-title">
                  ...
                  <div className="issues-title-details">
                    #{issue.number} opened {Date(issue.created_at)} hours ago by {issue.user.login}
                  </div>
                </div>
              </div>
              {issue.comments > 0 &&
